...
(async () => {
  ...
  const randomIsWhite = Math.random() < 0.5;
  const orientation = randomIsWhite ? "white" : "black";
  ...
  rooms.set(roomId, {
    ...
    players: [{
      ...
      color: orientation, // 💡 Színt tároljuk is
    }]
  });
  ...
  // ⚪⚫ A második játékos az ellenkező színt kapja
  ...
  const updatedRoom = {
    ...
    players: [
      ...room.players,
      { id: socket.id, username: socket.data?.username, color: secondColor },
    ],
  };
